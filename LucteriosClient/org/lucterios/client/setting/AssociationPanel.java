...
public class AssociationPanel extends JAdvancePanel implements TableModel {
	...
	class AssociationEditor extends JDialog {
		...
		public AssociationEditor(JDialog aOwner, String aExtName,
				String aAppliName) {
			super(aOwner);
			Initial();
			setTitle("Modifier");
	...
	protected void btn_ModActionPerformed(ActionEvent evt) {
		...
		AssociationEditor edt = new AssociationEditor(mOwnerFrame,
				(String) getValueAt(row, 0), (String) getValueAt(row, 1));
		edt.setVisible(true);
		refreshGrid();
	...
	protected void btn_AddActionPerformed(ActionEvent evt) {
		...
		edt.setVisible(true);
		refreshGrid();
		cmp_tbl.setRowSelectionInterval(mExtAssociation.size() - 1,
				mExtAssociation.size() - 1);
	}
