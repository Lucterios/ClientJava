...
public class TextCode extends JPanel implements KeyListener, CaretListener {
	...
	public void setValue(String aText) {
		...
		getEditor().getDocument().addUndoableEditListener(
				popupListener.getUndo());
	...
	public void setEnabled(boolean aEnabled) {
		...
		if (popupListener.getPopup() != null) {
			...
			for (int idx = 0; idx < popupListener.getPopup()
					.getComponentCount(); idx++)
	...
	public void setTabs(int charactersPerTab) {
		...
		getEditor().getStyledDocument().setParagraphAttributes(0, length,
				attributes, true);
	}
		...
		public void paintComponent(Graphics g) {
			...
			if (mFirstLine > 0) {
				...
				for (int num = 0; num <= (1 + (clip.y + clip.height) / h); num++)
					g.drawString(Integer.toString(num + mFirstLine), 0,(int)((num + 0.75)* h));
	...
	private void invertFocusTraversalBehaviour() {
		Set<AWTKeyStroke> forwardKeys = this
				.getFocusTraversalKeys(KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS);
		Set<AWTKeyStroke> backwardKeys = this
		...
		final int ctrlShiftMask = KeyEvent.SHIFT_MASK
				+ KeyEvent.SHIFT_DOWN_MASK + ctrlMask;
	...
	public void keyReleased(KeyEvent e) {
		if (m_StringSize > 0) {
			...
			if ((current_text.length() <= m_StringSize)) {
				...
				current_text = current_text.substring(0, mark)
						+ current_text.substring(dot);
